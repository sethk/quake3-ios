...
void MSG_WriteData( msg_t *buf, const void *data, int length ) {
	int i;
	for(i=0;i<length;i++) {
		MSG_WriteByte(buf, ((qbyte *)data)[i]);
	}
}
...
void MSG_WriteDeltaEntity( msg_t *msg, struct entityState_s *from, struct entityState_s *to, 
	...
	for ( i = 0, field = entityStateFields ; i < numFields ; i++, field++ ) {
		fromF = (int *)( (qbyte *)from + field->offset );
		toF = (int *)( (qbyte *)to + field->offset );
		if ( *fromF != *toF ) {
			lc = i+1;
		}
	}
...
void MSG_initHuffman() {
	...
	for(i=0;i<256;i++) {
		for (j=0;j<msg_hData[i];j++) {
			Huff_addRef(&msgHuff.compressor,	(qbyte)i);			// Do update
			Huff_addRef(&msgHuff.decompressor,	(qbyte)i);			// Do update
		}
	}
}
