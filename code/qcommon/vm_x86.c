...
static	qbyte	*buf = NULL;
static	qbyte	*jused = NULL;
static	int		compiledOfs = 0;
static	qbyte	*code = NULL;
static	int		pc = 0;
...
systemCall:
	...
	*(int *)((qbyte *)currentVM->dataBase + programStack + 4) = syscallNum;
//VM_LogSyscalls(  (int *)((qbyte *)currentVM->dataBase + programStack + 4) );
	*(opStack+1) = currentVM->systemCall( (int *)((qbyte *)currentVM->dataBase + programStack + 4) );
...
void callAsmCall(void)
{
	...
	*(int *)((qbyte *)currentVM->dataBase + callProgramStack + 4) = callSyscallNum;
//VM_LogSyscalls(  (int *)((qbyte *)currentVM->dataBase + programStack + 4) );
	*(callOpStack2+1) = currentVM->systemCall( (int *)((qbyte *)currentVM->dataBase + callProgramStack + 4) );
...
void VM_Compile( vm_t *vm, vmHeader_t *header ) {
	...
	pc = 0;
	instruction = 0;
	code = (qbyte *)header + header->codeOffset;
	compiledOfs = 0;
	...
	while ( instruction < header->instructionCount ) {
		...
		case OP_CONST:
			...
			if (code[pc+4] == OP_LOAD1) {
				...
				EmitString( "BB" );		// mov	ebx, 0x12345678
				Emit4( (Constant4()&vm->dataMask) + (int)vm->dataBase);
				EmitString( "0F B6 03" );	// movzx	eax, qbyte ptr [ebx]
				EmitCommand(LAST_COMMAND_MOV_EDI_EAX);		// mov dword ptr [edi], eax
				pc++;						// OP_LOAD4
				instruction += 1;
				break;
...
//					Emit4( vm->dataMask );
//				}
				EmitString( "88 83" );		// mov qbyte ptr [ebx+0x12345678], eax
				Emit4( (int)vm->dataBase );
				EmitCommand(LAST_COMMAND_SUB_DI_4);		// sub edi, 4
				pc++;						// OP_STORE4
				instruction += 1;
		...
		case OP_LOAD1:
			EmitMovEBXEDI(vm, vm->dataMask);
			EmitString( "0F B6 83" );	// movzx eax, qbyte ptr [ebx + 0x12345678]
			Emit4( (int)vm->dataBase );
			EmitCommand(LAST_COMMAND_MOV_EDI_EAX);		// mov dword ptr [edi], eax
			break;
		case OP_STORE4:
...
//			Emit4( vm->dataMask );
		...
		case OP_CVFI:
		  ...
		  EmitString( "D9 07" );		// fld dword ptr [edi]
			...
			break;
		case OP_SEX8:
			EmitString( "0F BE 07" );	// movsx eax, qbyte ptr [edi]
			EmitCommand(LAST_COMMAND_MOV_EDI_EAX);		// mov dword ptr [edi], eax
			break;
		case OP_SEX16:
