...
===============
*/
static	void R_ColorShiftLightingBytes( qbyte in[4], qbyte out[4] ) {
	int		shift, r, g, b;
...
static	void R_LoadLightmaps( lump_t *l ) {
	qbyte		*buf, *buf_p;
	int			len;
	MAC_STATIC qbyte		image[LIGHTMAP_SIZE*LIGHTMAP_SIZE*4];
	int			i, j;
...
static void ParseFace( dsurface_t *ds, drawVert_t *verts, msurface_t *surf, int *indexes  ) {
	...
	for ( i = 0 ; i < numPoints ; i++ ) {
		...
		R_ColorShiftLightingBytes( verts[i].color, (qbyte *)&cv->points[i][7] );
	}

	indexes += LittleLong( ds->firstIndex );
	for ( i = 0 ; i < numIndexes ; i++ ) {
		((int *)((qbyte *)cv + cv->ofsIndices ))[i] = LittleLong( indexes[ i ] );
	}
...
void RE_LoadWorldMap( const char *name ) {
	...
	s_worldData.dataSize = (qbyte *)ri.Hunk_Alloc(0, h_low) - startMarker;
