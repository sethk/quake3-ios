...
//Com_Printf ("%i < %i < %i\n", s_soundtime, s_paintedtime, s_rawend);
	...
	else if (s_channels == 1 && width == 1)
	{
		...
		for (i=0 ; ; i++)
		{
			...
			dst = s_rawend&(MAX_RAW_SAMPLES-1);
			s_rawend++;
			s_rawsamples[dst].left = (((qbyte *)data)[src]-128) * intVolume;
			s_rawsamples[dst].right = (((qbyte *)data)[src]-128) * intVolume;
		}
	}
...
void S_UpdateBackgroundTrack( void ) {
	int		bufferSamples;
	int		fileSamples;
	qbyte	raw[30000];		// just enough to fit in a mac stack frame
	int		fileBytes;
	int		r;
