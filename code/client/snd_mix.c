...
__declspec( naked ) void S_WriteLinearBlastStereo16 (void)
{
 ...
 push edi
 push ebx
 mov ecx,dword ptr[snd_linear_count]
 mov ebx,dword ptr[snd_p]
 mov edi,dword ptr[snd_out]
LWLBLoopTop:
 mov eax,dword ptr[-8+ebx+ecx*4]
 sar eax,8
 cmp eax,07FFFh
 jg LClampHigh
 cmp eax,0FFFF8000h
...
/*	int						data;
	...
	if (!ch->doppler) {
		samples = (qbyte *)chunk->sndChunk + sampleOffset;
		for ( i=0 ; i<count ; i++ ) {
			data  = mulawToShort[*samples];
			samp[i].left += (data * leftvol)>>8;
			samp[i].right += (data * rightvol)>>8;
			samples++;
			if (samples == (qbyte *)chunk->sndChunk+(SND_CHUNK_SIZE*2)) {
				chunk = chunk->next;
				samples = (qbyte *)chunk->sndChunk;
			}
		}
	} else {
		ooff = sampleOffset;
